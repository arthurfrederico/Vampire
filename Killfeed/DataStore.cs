...
public class DataStore
{
	...
	private static void AnnounceKill(PlayerStatistics victimUser, PlayerStatistics killerUser, int lostStreakAmount)
	{
		...
		var message = lostStreakAmount > Settings.AnnounceKillstreakLostMinimum
			? $"{killerName} finalizou a kill streak de {victimName} de {Markup.Secondary(lostStreakAmount) } vitimas!"
			: $"{killerName} matou {victimName}!";

		var killMsg = killerUser.CurrentStreak switch
		{
			5 => $"<size=18>{killerName} ta numa killing spree!",
			10 => $"<size=19>{killerName}  rampage!",
			15 => $"<size=20>{killerName}  dominating!",
			20 => $"<size=21>{killerName}  unstoppable!",
			25 => $"<size=22>{killerName}  godlike!",
			30 => $"<size=24>{killerName}  WICKED SICK!",
			_ => null
		};
