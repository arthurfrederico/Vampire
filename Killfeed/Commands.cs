...
{
	[Command("killfeed tabela", shortHand: "kf top")]
	...
	public void KillfeedCommand(ChatCommandContext ctx)
	{
		...
		{
			throw ctx.Error($"Você ainda não tem nenhum status!");
		}
		...
		sb.AppendLine($"Mortes: {Markup.Highlight(stats.Deaths)}");
		sb.AppendLine($"Streak Atual: {Markup.Highlight(stats.CurrentStreak)}");
		sb.AppendLine($"Maior Streak: {Markup.Highlight(stats.HighestStreak)}");
